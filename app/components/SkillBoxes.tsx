    ...
    return (
        <div className="w-full pt-16 flex flex-col gap-16">
          ...
          <div className="min-h-screen py-4 flex flex-col gap-16 items-center">
            <div className="w-full text-sm md:text-xl grid grid-cols-2 md:grid-cols-4 text-center gap-8">
                {skills.map((skill, index) => <GradientBox key={index} index={index} focused={focused} setFocused={setFocused}><skill.icon className="h-16 w-16"/><h4 className={inter.className}>{skill.title}</h4></GradientBox>)}
            </div>
